...
#+filetags: :linux:qemu:virtualizacion:
...
- Archivez .qcow2 :: Hard Disk Drive (qemu format).
These archives are basically the hard disk drive. So we are basically just call this within =QEMU=.
